...
def focus(im_series, shift_facter, depth=None):
	"""
	...
	shift_vec = np.linspace(start=(-np.floor(s / 2)) * shift_facter, stop=(np.ceil(s / 2) - 1) * shift_facter, num=s)
